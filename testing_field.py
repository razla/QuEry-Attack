...
models_names = ['custom', 'densenet121', 'densenet169', 'mobilenet_v2', 'resnet18']
...
if __name__ == '__main__':
    ...
    for i, (x, y) in enumerate(zip(x_test, y_test)):
        ...
        if correctly_classified(model, x, y) and count < n_images:
            ...
            adv, n_queries = EvoAttack(model=model, img=x, label=y, metric='linf', delta=delta, perturbed_pixels=1,
    ...
    accuracy = np.sum(np.argmax(predictions, axis=1) == np.argmax(y_test, axis=1)) / len(y_test)
    print("Accuracy on Wasserstein test examples: {}%".format(accuracy * 100))
